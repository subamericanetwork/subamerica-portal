-- =====================================================
-- PHASE 1: FOUNDATION SETUP FOR LIVEPUSH INTEGRATION
-- =====================================================

-- 1. CREATE ROLE SYSTEM (CRITICAL SECURITY)
-- =====================================================
CREATE TYPE public.app_role AS ENUM ('admin', 'artist', 'moderator');
...
-- Grant first admin role to colleen-anthony
...
-- 2. CREATE SUBSCRIPTION TIER SYSTEM
-- =====================================================
...
-- 3. LIVEPUSH ARTIST PERMISSIONS
-- =====================================================
CREATE TABLE public.livepush_artist_permissions (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
-- 4. LIVEPUSH VIDEO SYNC
-- =====================================================
CREATE TABLE public.livepush_videos (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  artist_id UUID REFERENCES artists(id) NOT NULL,
  ...
  -- Copyright detection (for future use)
...
-- 5. STREAM PLAYLISTS (PRE-BROADCAST)
-- =====================================================
CREATE TABLE public.stream_playlists (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  artist_id UUID REFERENCES artists(id) NOT NULL,
...
-- 6. UPDATE EVENTS TABLE FOR LIVE STREAMS
-- =====================================================
ALTER TABLE public.events
ADD COLUMN IF NOT EXISTS livepush_stream_id TEXT,
ADD COLUMN IF NOT EXISTS rtmp_url TEXT,
ADD COLUMN IF NOT EXISTS stream_key TEXT,
ADD COLUMN IF NOT EXISTS livestream_status TEXT DEFAULT 'scheduled',
ADD COLUMN IF NOT EXISTS multistream_destinations JSONB DEFAULT '[]'::jsonb,
...
-- 7. DJ-SPECIFIC FEATURES: TRACKLISTS
-- =====================================================
CREATE TABLE public.stream_tracklists (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
-- 8. DJ-SPECIFIC FEATURES: TIMESTAMP MARKERS
-- =====================================================
CREATE TABLE public.stream_markers (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  stream_id UUID REFERENCES stream_playlists(id) ON DELETE CASCADE,
  event_id UUID REFERENCES events(id) ON DELETE CASCADE,
...
-- 9. DJ-SPECIFIC FEATURES: DONATION ALERTS
-- =====================================================
...
-- 11. ADMIN NOTIFICATION PREFERENCES
-- =====================================================
