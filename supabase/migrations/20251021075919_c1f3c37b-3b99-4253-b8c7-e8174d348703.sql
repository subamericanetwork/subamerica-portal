-- Fix ambiguous column reference in approve_artist_application function
...
BEGIN
  ...
  -- Update application status (fix: qualify the column name to avoid ambiguity)
  UPDATE artist_applications
  SET 
    status = 'approved',
    reviewed_at = NOW(),
    reviewed_by = admin_id,
    artist_applications.admin_notes = approve_artist_application.admin_notes
  WHERE artist_applications.id = application_id;
